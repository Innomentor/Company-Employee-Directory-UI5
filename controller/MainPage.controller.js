        ...
        return Controller.extend("project1.controller.MainPage", {
            ...
            onDelete: function (oEvent) {
                ...
                MessageBox.confirm("Do you really want to delete the selected Customer??", {
                    ...
                    onClose: function (oAction) {
                        if (oAction === "OK") {
                            var sPath1 = that.byId("tableID1").getSelectedItems()[0].getBindingContextPath();
                            var data = that.getOwnerComponent().getModel("oDataModel").getProperty(sPath1);
                            that.onRadioButton();
                            data.remove("/Customers ('" + custId + "')"), {
            ...
            onCancel: function(oEvent){
                ...
                MessageBox.confirm("Do you really want to delete the selected Employee??", {
                    ...
                    onClose: function (oAction) {
                        if (oAction === "OK") {
                            var sPath = that.byId("tableID").getSelectedItems()[0].getBindingContextPath();
                            var data = that.getOwnerComponent().getModel("oDataModel").getProperty(sPath);
                            that.onRadioButton();
                            data.remove("/Employees (" + empId + ")"), {
                                sucess: function (response) {
                                    console.log(response);
                                },
                                error: function (error) {
